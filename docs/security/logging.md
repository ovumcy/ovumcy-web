...
- `AUDIT_LOG_ENABLED=true` — the runtime emits per-action security-event lines to stderr via the Go standard `log` package. Each line includes the action name, outcome, request method, **sanitized** request path (concrete date parameters are replaced with `:date` and other identifiers are similarly masked), response format, and — for authenticated requests — `user_id` and role. Example:
  ...
  An action that changes health data carries the two further fields shown above: `domain="health_data"`, and a `target` naming the scope it touched (`day_entry`, `symptom`, `cycle_start`, `cycle_settings`, `tracking_settings`, `reminder_settings`, `webhook_settings`, `calendar_feed`, `timezone`, and — for the two actions that erase everything — `account_data` for a full data wipe, `account` for an account deletion; the eleven values are the `healthMutationKind` declarations in `internal/api`). The target is a fixed designator chosen per action, never an identifier, an email, or submitted text, so an incident review can filter on `domain="health_data"` to select the data-changing actions, erasures included.
...
One further always-on group names no `user_id` but does expose the destination **hostname** of an owner's configured webhook: the delivery skip/failure lines (`webhook delivery skipped: reason=… host=…`, `webhook delivery failed: reason=… host=…`). The host is a fragment of the otherwise-encrypted `webhook_url`; host-only masking — never the path, query or userinfo — is test-pinned (`TestWebhookDeliveryLogsHostOnly`), and the section's retention advice covers these lines too.
