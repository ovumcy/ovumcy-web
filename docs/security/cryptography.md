...
| Calendar-feed verifier MAC (`users.calendar_feed_verifier_mac`) | `HKDF-SHA256(SECRET_KEY, salt="ovumcy.calendar-feed-token.salt.v1", info="ovumcy.calendar-feed-token.key.v1")` → `HMAC-SHA256` over the token's `(selector, verifier)` | **Every armed calendar feed stops verifying.** A stored MAC that no longer matches is refused outright and is deliberately **not** re-checked against the row's bcrypt hash, so the subscribed calendar client starts receiving `404`. This is intentional: healing from bcrypt would keep a long-lived bearer capability alive across a key rotation and would put every wrong verifier back on the ~265 ms bcrypt path. Each owner re-generates the subscribe URL from settings and re-subscribes. A feed armed before migration 032 has no MAC yet and is unaffected until its first successful poll. |
...
The sealed-cookie and field-crypto HKDF label pairs live next to the shared AEAD primitive (`SealedCipher` in `internal/security/sealed_cipher.go`); the calendar-feed pair lives with its own primitive (`internal/security/calendar_feed_mac.go`). Each purpose derives a distinct key, so a value from one purpose never verifies or opens under another. The sealed-cookie codec in `internal/api/secure_cookie_codec.go` adds only cookie framing on top: the name→AAD mapping, the `v2` version envelope, and base64url transport.
