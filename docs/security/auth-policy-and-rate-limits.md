...
| `GET/HEAD /calendar/feed/:token.ics` (HEAD only spends this budget — see note below) | 20 requests / 1 minute | `RATE_LIMIT_CALENDAR_FEED_MAX`, `RATE_LIMIT_CALENDAR_FEED_WINDOW` |
...
A HEAD request to the feed counts against this budget without being served by it: on the current route table, fiber only appends a GET route's auto-generated HEAD copy after every directly-registered `Use` middleware — the terminal `NotFound` catch-all included — already occupies the HEAD stack, so HEAD reaches that catch-all's 404 before `ServeCalendarFeed` ever runs (`cmd/ovumcy/calendar_feed_route_shape_test.go`). That ordering is site-wide, not specific to this route, and fixing it is separate work; until then, "HEAD" in the row above means "rate-limited", not "answers the feed".
