...
- Login attempts: 8 failures / 15 minutes. The retained-but-unreachable `/auth/oidc/link-confirm` handler still checks a submitted password against this same budget when driven directly, as the regression suite does with a hand-sealed cookie — but the OIDC callback has not minted the pending-link cookie that handler needs since issue #701, so no request reaching it from a real browser flow gets past the cookie read to spend this budget at all; see *Where a link is actually created* in [docs/security/oidc-and-sessions.md](oidc-and-sessions.md).
