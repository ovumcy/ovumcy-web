...
- keep `OIDC_CLIENT_SECRET` private just like `SECRET_KEY` and `.env`; `OIDC_CLIENT_SECRET_FILE` takes a secret file path instead, the same pattern as `SECRET_KEY_FILE` below (`OIDC_CLIENT_SECRET` wins if both are set).
  Neither is read while `OIDC_ENABLED` is off, so a stale secret file path does not block the boot of an instance that has OIDC disabled;
...
| No fence available at all (no mount, `CALENDAR_FEED_FENCE_PATH` unset, or the path not writable) | Every armed calendar feed is disarmed **on every start**, and the startup log says so and names the variable. The instance still boots and everything else works; a database failure during the pass — the disarm, the marker drop, or the stamp — fails the start, as any database failure does. The calendar feed is effectively unavailable until you mount a fence. The database also records that this instance started without a fence, which is what the row below reads. Such a start also **drops the database half of the marker**, so the first start that has the fence back reads the file against a database half it no longer matches and disarms once more before re-arming — the "database restore, or a fence volume that was recreated" row above — even when nothing was restored. A fence that fails only for a moment pays this too: a container that starts before its fence volume has mounted disarms every armed feed at that start and once more at the next healthy one, where before it would have found both halves agreeing. Start the volume first, or expect two rounds of subscribe URLs from one flaky mount. |
| First start **with** a fence whose **file is empty too** (an instance that had only ever started without one, or a brand-new fence volume) over a database stamped as having run unfenced | Every armed calendar feed is disarmed once, and the startup log says the fence was armed for the first time on a database that had run without it. Expected, not a fault — see the paragraph after this table — and the starts after it disarm nothing. |
| `CALENDAR_FEED_FENCE_PATH` set to a relative path | Refused at startup; the server does not boot. See the paragraph after this table. |
