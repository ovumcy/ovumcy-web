...
6. Otherwise, if a verified email claim matches an existing local account, Ovumcy **neither links the two nor signs the user in**. It redirects to `/auth/oidc/link-confirm` and requires that account's own password — plus a valid TOTP code when the account has 2FA enabled — before the `(issuer, subject)` link is stored. Without this gate, any provider able to assert an existing user's email address could take over their account. The invariant and its mechanics — the sealed pending cookie and its TTL, the retry behaviour, the refusal when another account claimed the same `(issuer, subject)` meanwhile, and the refusal for accounts with local auth disabled — are owned by [docs/security/oidc-and-sessions.md](security/oidc-and-sessions.md); this page keeps the operator-facing flow.
