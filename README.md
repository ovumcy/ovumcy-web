...
This README describes the current `main` branch. The latest tagged release is `v0.8.5`.
...
Uses the prebuilt image from GHCR pinned to the latest tagged release by default (`ghcr.io/ovumcy/ovumcy-web:v0.8.5`).
...
```bash
OVUMCY_IMAGE=ghcr.io/ovumcy/ovumcy-web:v0.8.5 docker compose up -d
```
