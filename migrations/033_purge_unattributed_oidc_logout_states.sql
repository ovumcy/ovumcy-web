-- Purge the pre-031 oidc_logout_states rows that account deletion cannot reach.
--
...
-- the data to be gone.
--
-- Deleting them closes that gap for good. Afterwards every row in the table
-- carries a user_id, so the explicit per-user delete covers the whole table and
-- erasure is complete at the moment it is requested rather than a week later.
--
