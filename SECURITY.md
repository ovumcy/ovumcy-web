...
| The `.ics` feed has its own per-IP budget, strictly tighter than the `/api` catch-all, because every well-formed request costs a bcrypt compare — the feed limiter must never inherit the cheap-read budget | `TestLoadRuntimeConfigGivesCalendarFeedItsOwnBudget`, `TestCalendarFeedLimiterUsesItsOwnBudgetNotTheAPIBudget` in [cmd/ovumcy/main_test.go](cmd/ovumcy/main_test.go) and [cmd/ovumcy/rate_limit_negotiation_test.go](cmd/ovumcy/rate_limit_negotiation_test.go) |
