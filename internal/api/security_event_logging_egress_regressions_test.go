...
// securityEventLine isolates the ONE audit line matching an action/outcome pair.
// Every assertion below runs against that line rather than against the buffer:
// a single request emits several security events, so a whole-buffer substring
// check passes as long as any line carries the field — including a line from a
// different action that was already tagged correctly.
func securityEventLine(t *testing.T, logOutput string, action string, outcome string) string {
	...
	if len(matches) > 1 {
		t.Fatalf("expected exactly one audit line for %s, got %d: %q", header, len(matches), matches)
	}
	return matches[0]
}
