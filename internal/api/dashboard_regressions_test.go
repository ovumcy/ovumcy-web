...
// (stats_overview_contract_test.go): the same CD32 fixture, read through the
// dashboard's rendered ovulation slot rather than the JSON payload, so the two
...
func TestDashboardNamesALateShiftAfterTheProjectedNextPeriodStart(t *testing.T) {
	...
	// conceive (resolveDashboardTimingFrame, dashboard_view_service.go); the
	// suppression math this fixture exercises is otherwise identical to the
	// JSON-API test it mirrors.
	...
	confirmedDay := today.AddDate(0, 0, -3)
