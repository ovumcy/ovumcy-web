...
func TestIsResetPasswordTokenValid(t *testing.T) {
	...
	validToken, err := BuildPasswordResetToken(secret, 7, "$2a$10$testhashvaluefortokenclaims", 1, PasswordResetTokenPurposeRecovery, 30*time.Minute, now)
	if err != nil {
		t.Fatalf("BuildPasswordResetToken returned error: %v", err)
	}

	expiredToken, err := BuildPasswordResetToken(secret, 7, "$2a$10$testhashvaluefortokenclaims", 1, PasswordResetTokenPurposeRecovery, time.Minute, now.Add(-2*time.Minute))
	if err != nil {
