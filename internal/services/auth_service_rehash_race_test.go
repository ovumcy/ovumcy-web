...
func rehashRaceDrivers(t *testing.T, run func(t *testing.T, database *gorm.DB)) {
	t.Helper()

	t.Run("sqlite", func(t *testing.T) {
		run(t, newTwoOwnerIntegrationDatabase(t, "rehash-race"))
	})
	t.Run("postgres", func(t *testing.T) {
		run(t, newTwoOwnerIntegrationDatabaseWithConfig(t, db.Config{
...
// TestRehashLosesToAPasswordChangeBetweenCompareAndWrite is the race: the
// login has proven the old password against the old hash, and the change lands
// before the rehash write.
func TestRehashLosesToAPasswordChangeBetweenCompareAndWrite(t *testing.T) {
	rehashRaceDrivers(t, func(t *testing.T, database *gorm.DB) {
...
// race case above: the same interleaving against a writer that ignores the
// verified hash must bring the replaced password back. If it does not, the
// race case is not measuring the predicate and its green means nothing.
func TestRehashRaceDetectsAnUnconditionalUpgrade(t *testing.T) {
	rehashRaceDrivers(t, func(t *testing.T, database *gorm.DB) {
...
// TestRehashBeforeAPasswordChangeKeepsTheChange is the reverse order: the
// upgrade applies, then the change replaces it.
func TestRehashBeforeAPasswordChangeKeepsTheChange(t *testing.T) {
	rehashRaceDrivers(t, func(t *testing.T, database *gorm.DB) {
...
func TestRehashUpgradesAnUncontendedStaleHash(t *testing.T) {
	rehashRaceDrivers(t, func(t *testing.T, database *gorm.DB) {
