...
class `e2e.md` documents (#620). The seed is now anchored at day 1 of the current month and the
view navigates to that month explicitly, so the whole window sits inside one month's grid on every
run date. No production behavior changes.
